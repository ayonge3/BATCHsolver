	...
	if TAPobject_data.finite_difference_trans_sensitivty == True:
		...
		for k in TAPobject_data.reactor_species.gasses:
			...
			for j in range(0,pulse_number):
				...
				for n in qoi:
					transient_data[k][j][n] = []
		for jnum,j in enumerate(qoi):
			TAPobject_data.output_name = 'df_flux'
			old_value = copy.deepcopy(eval(j))
			
			if 'delay' in j:
				new_value = old_value+0.0001
			elif 'dG' in j:
				new_value = old_value+0.0001
			else:
		...
		save_object(transient_data,'./'+original_name+'/TAP_derivatives.json')
		deriviative_information = read_transient_sensitivity('./'+original_name+'/TAP_derivatives.json')
		...
		try:
			...
			print('failed post derivative analysis')